/**
 * Cleanup fanfic novels that slipped in from specific Wattpad Reading Lists.
 *
 * Why:
 * - Even with on-ingest filtering, some fandom/doujinshi entries can slip through.
 * - This script deletes ONLY novels whose originalLink is in the provided lists.
 *
 * Usage:
 *   node scripts/cleanupFanficFromWattpadLists.js
 *   node scripts/cleanupFanficFromWattpadLists.js https://www.wattpad.com/list/123 ...more
 */
...
// Keep this list conservative but useful for obvious fanfic/doujin/fandom content
...
function isFanficNovel(novel) {
  ...
  for (const kw of FANFIC_KEYWORDS) {
    ...
    if (!kwNorm) continue;
    if (!kwNorm.includes(' ')) {
      if (tokens.has(kwNorm)) return { isFanfic: true, keyword: kw };
      continue;
    }
    if (hasWholePhrase(combinedNorm, kwNorm) || hasWholePhrase(tagsNorm, kwNorm)) {
      return { isFanfic: true, keyword: kw };
    }
  }
...
async function main() {
  ...
  for (const novel of candidates) {
    const res = isFanficNovel(novel);
    if (res.isFanfic) {
      toDelete.push({ _id: novel._id, title: novel.title, keyword: res.keyword });
    }
  }

  console.log(`🧹 Will delete ${toDelete.length} fanfic novels from these lists`);
